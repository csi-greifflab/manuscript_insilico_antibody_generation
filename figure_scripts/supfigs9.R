...
get_epitope_count_distr = function(){
    ...
    tinfile = 'eleven_outfiles/antigens_feature_transfer_generated_nsamples_10k_count.csv'
    ...
    outpng('fig', 'S10',widt=20, height=10)
}
...
get_epitope_count_overlap = function(){
    ...
    print(outdf)
    outname = 'eleven_outfiles/antigens_feature_native_transfer_generated_nsamples_count_overlap.csv'
    write.csv(outdf, outname, row.names=FALSE)
}


plot_epitope_count_overlap = function() {
    infile = 'eleven_outfiles/antigens_feature_native_transfer_generated_nsamples_count_overlap.csv'
    df = read_csv(infile)
    ...
    ggplot(data=gdf, mapping=aes(x=ntrainseq, y=countvalue)) +
        ...
        geom_text(data=meddf, mapping=aes(x=ntrainseq, y=-1, color=counttype,label=medval),
        position=position_dodge2(width=1)) +
        # geom_point(mapping=aes(color=counttype),size = 3, shape = 21, position =
        #     position_jitterdodge()) +
        facet_wrap(~antigen, nrow=2) +
        scale_color_manual(values=c('black', my_base_color[2], nativecol, 'black', my_base_color[2]))+
        scale_fill_manual(values=c('black', my_base_color[2], nativecol, 'black', my_base_color[2]))+
        labs(x='Number of training CDR-H3 sequences',
        y='Overlap of epitopes recognized by\nnative and generated CDR-H3 sequences') +
        theme(legend.title=element_blank())
    outpng('fig', 'S10b', width=15)
