...
get_epitope_cor = function(){
    ...
    for (antigen in antigens) {
        ...
        for (nsample in nsamples){
            for(ntrainseq in ntrainseqs){
                ...
                mgdf[is.na(mgdf)] = 0
                mtdf = merge(ftdf, sndf, by='AGbindPositions')
                mtdf[is.na(mtdf)] = 0
                print(mtdf)
                print(dim(mtdf))
                print(mgdf)
                ...
                gcordf = data.frame(antigen = antigen, corval = gcorval, nsample=nsample, ntrainseq=ntrainseq,
                        crossstatus='-T')
                outdf = rbind(outdf, tcordf,gcordf)
                print(outdf)
            }
        }
    }
    ...
    outdf$ntrainseq = factor(outdf$ntrainseq, levels=c('ntrain700',
                                                        ...
                                                        'ntrain60000',
                                                        'ntrain70000'))
    outname = 'eleven_outfiles/antigens_feature_native_transfer_generated_nsamples_count_cor.csv'
