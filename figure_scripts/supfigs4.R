...
affinity_nsamples_transfer_distr= function(){
    generated_infile = 'eleven_outfiles/antigens_affinity_ntrainingsamples_generated_sampled.csv'
    transfer_infile = 'eleven_outfiles/antigens_affinity_ntrainingsamples_transfer_generated_sampled.csv'
    ...
    for (model in unique(df$model)){
        ...
        if (grepl('impoverished', model)){
            model_short=sprintf('-T_%s_ntrain%s', model_parts[2],model_parts[3])
            crossstatus = '-T'
        }
        ...
        print(model_short)
        mdf$model_short = rep(model_short, dim(mdf)[1])
        mdf$crossstatus = rep(crossstatus, dim(mdf)[1])
        outdfs = rbind(outdfs, mdf)
        # print(outdfs)
    ...
    print(outdfs)
    outdfs$model_short[outdfs$model_short=='-T_opulent'] = '-T_ntrain70000'
    outdfs$model_short = factor(outdfs$model_short, levels=c(sprintf('-T_sample%s_ntrain700', c(0:4)),
        sprintf('+T_sample%s_ntrain700', c(0:4)),
        sprintf('-T_sample%s_ntrain7000', c(0:4)),
        sprintf('+T_sample%s_ntrain7000', c(0:4)),
        '-T_ntrain70000',
        '-T_native'))
    outdfs = fixfive(outdfs)
    meddf = outdfs %>% group_by(antigen, model_short, crossstatus) %>% summarize(medval=median(Energy))
    print(meddf)
    meddf$medval = round(meddf$medval,2)
    ggplot(data=outdfs) +
        geom_density_ridges(mapping= aes(x=Energy, y = model_short, color=crossstatus, fill=crossstatus)) +
        geom_text(data=meddf, mapping=aes(x=medval, y=model_short, label=medval)) +
        facet_wrap(~antigen, nrow=2) +
        labs(x='Affinity (Energy)', y='Models') +
        theme(legend.title = element_blank(),
            legend.text= element_text(size=17),
            axis.title = element_text(size=17)) +
        scale_fill_manual(values=c('red', 'gray')) +
        scale_color_manual(values=c('red', 'gray'))
    outpng('fig', 'S4', width=17, height=11)
    stop()
...
affinity_nsamples2 = function(){
    ...
    print(meddf)
    meddf = fixfive(meddf)
    meddf$model_name[meddf$model_name == 'opulent'] = 'datarich'
    ...
    ggplot(data=df) +
        ...
        scale_color_manual(values=c(nativecol,igemcol)) +
        scale_fill_manual(values = c(nativecol, igemcol))
    outpng('affinity','ntrainingsamples_distr', width=25, height=15)
}


# run stuff
affinity_nsamples_transfer_distr() # panelB top
